...
class DrivingClient(DrivingController):
    ...
    def control_driving(self, car_controls, sensing_info):
        ...
        # 상대 차량의 장애물 여부 판단
        opponents = sensing_info.opponent_cars_info

        for opponent in opponents:
            if opponent['dist'] > 0 and opponent['speed'] < spd:
                obstacles.append({'dist': opponent['dist'], 'to_middle': opponent['to_middle']})

        obstacles = sorted(obstacles, key=lambda obstacle: abs(obstacle['dist']))
        ...
        if sensing_info.collided or (sensing_info.lap_progress > 0.5 and self.accident_step == 0 and abs(spd) < 1.0):
